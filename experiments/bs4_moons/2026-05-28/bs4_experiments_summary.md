...
| **`ancestral_mc_td_lambda`** (re-tuned, fixed estimator — see update below) | on-policy "other A" | −5.67 | −4.55 | **16,999** | **−5.67** |
| `ancestral_mc_td_lambda` (t56, *original buggy estimator*) | on-policy "other A" | −6.33 | −5.96 | 32,999 | −7.35 |
...
| `ancestral_mc_td_lambda` (t56, original) | smc_value = **k·V + l·t·r**, k=0.057, l=0.0046, mc=7, n_steps=10, off_frac=0.34, λ=0.45, lr=3.7e-4, grad_decay=4.6e-5 |
| `ancestral_mc_td_lambda` (re-tuned, t22) | smc_value = **0.710·r + 0.0105·V**, mc=21, n_steps=19, off_frac=0.034, λ=0.93, lr=1.6e-4, **quad** loss, no grad-decay |
...
## Update (2026-05-29): `ancestral_mc_td_lambda` estimator fix + re-tune

A correctness bug was found in `ancestral_mc_td_lambda`'s backward pass: for `lambda_eff > 0` the multi-step term mis-indexed the resampling weights (grouping by the *next* resample step instead of the one that created the particle, and decoupling a product of expectations into a product of means). The net effect was an **O(1), twist-dependent bias** leaking `log_tau` into the regression target — only `lambda_eff = 0` was unbiased. The fix averages the per-child product `w(child)·ρ̂(child)` over a parent's resampled copies (grouped by the resample that created the parent); only a small `O(dt)` SMC smoothing residual remains. See [`src/diffusion_rl/models/on_policy.py`](../../src/diffusion_rl/models/on_policy.py) (`ancestral_mc_td_lambda`) and the regression test `tests/models/test_on_policy.py::test_ancestral_mc_td_lambda_target_is_tau_independent_and_unbiased`.

A dedicated re-tune ([`optuna_amctl_bs4_sweep.py`](optuna_amctl_bs4_sweep.py)) was run under the fixed estimator with a **more general `log_tau`**: a non-negative linear combination of `{r, t·r, V, V_ema}` (each term independently toggled, coefficients log-uniform in [1e-3, 3]), alongside `λ_eff`, `mc_samples` (2–32), `n_steps`, `off_policy_frac`, `lr`, `grad_decay`, and `loss_type ∈ {quad, mse}`. Same LCB objective and BS=4 setup; 80-trial TPE+Hyperband sweep → top-3 × 5-seed confirm → 50k converge.

- **Result:** plateau **−6.33 → −5.67** (+10%), converges **~2× faster** (33k → 17k steps), final-LCB **−7.35 → −5.67** — which now narrowly *edges* `single_seed_td_lambda`'s −5.72 on the optimized metric and is the **fastest-converging method** of all. `single_seed_td_lambda` still holds the best raw plateau (−5.16); the corrected `ancestral_mc_td_lambda` closes most of the gap.
- **Winner (t22):** `log_tau = 0.710·r + 0.0105·V`, `λ_eff = 0.93`, `mc_samples = 21`, `n_steps = 19`, `off_frac = 0.034`, `lr = 1.6e-4`, **quad** loss, no grad-decay. Notably the winner leans on **heavy `λ`** (the multi-step path the bug used to corrupt) and **high `mc`** (the corrected duplicate-averaging rewards more particles) — both consistent with the fix being load-bearing.
- **quad ≫ mse:** all top-9 completed trials used `quad`; the best `mse` trial reached only −20.5 LCB vs −8.7 for the best `quad`, and TPE quickly stopped proposing `mse`. The log-space quadratic Bregman divergence is clearly the better loss here.
- **Caveat:** this re-tune ran on CUDA (RTX 3090 Ti), not the original Apple MPS; task, seeds, objective, and budgets are otherwise identical. (A small CUDA-safety fix to `OnPolicySMCDataset` was needed — it previously called `torch.mps.empty_cache()` on any non-CPU device.)
...
| `ancestral_mc_td_lambda` re-tune (fixed estimator, 2026-05-29) | `optuna_amctl_bs4_sweep_results.json` | [optuna_amctl_bs4_sweep.png](optuna_amctl_bs4_sweep.png) | `optuna_amctl.db` | `checkpoints/optuna_amctl_converge/amctl_t22_converge/` |
