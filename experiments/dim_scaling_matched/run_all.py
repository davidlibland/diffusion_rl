...
Each cell (run_cell.py) is a subprocess running all MM_N_SEEDS seeds in-process
(amortising the torch import); cells run MM_CONCURRENCY at a time on the one GPU.
Cells already fully done (summary with n == MM_N_SEEDS) are skipped; partially
done cells resume their remaining seeds.  Per-cell stdout -> results/log_<cell>.out.

Env: MM_DIMS="2,8,16,32,64,128,256,512"  MM_METHODS=...  MM_CONCURRENCY=3
     MM_N_SEEDS=30  (+ per-cell budget vars consumed by run_cell.py)
"""
...
DIMS = [int(x) for x in os.environ.get("MM_DIMS", "2,16,64,256").split(",")]
METHODS = os.environ.get(
    "MM_METHODS",
    "off_policy,single_seed_mc,single_seed_td_lambda,ancestral_mc_td_lambda",
).split(",")
CONC = int(os.environ.get("MM_CONCURRENCY", 4))
