"""
Alternating off-policy / on-policy training.

Instead of mixing samples within a batch (which OOMs), alternate entire
training phases: run K steps off-policy, then K steps on-policy, repeating.
Only one dataset is alive at a time.

Configurations:
  - offpolicy (baseline, 6000 steps)
  - osb_blend ws=3000 (previous best)
  - alternating K=50 steps, EMA decay=0.999
  - alternating K=50 steps, EMA decay=0.99
  - alternating K=100 steps, EMA decay=0.999
  - pure on-policy EMA decay=0.999 (for comparison)
"""
...
class AnalyticalValue(nn.Module):
    ...
    def _log_Z(self, m, v):
        cc = self.c.double(); d = 1.0+20.0*v
        return (-self.D/2.0*torch.log(d)+(-10*(m**2).sum(-1)+20*(m*cc).sum(-1)+200*v*(cc**2).sum())/d-10*(cc**2).sum())
...
def gmm_drift(xt,ts,a):
    ts=ts.reshape(-1,1);cond=get_cond_mix(xt,ts,_means.float().to(xt),_sigmas.float().to(xt),_weights_col.float().to(xt),a)
...
LR = 3e-3; LOADER_BATCH_SIZE = 256
WS_STEPS = 3000; ON_STEPS = 3000; TOTAL_STEPS = WS_STEPS + ON_STEPS
LOG_DIR = "lightning_logs/alternating_exp"
CKPT_DIR = "checkpoints/alternating_exp"
...
class TrajectoryEvalCallback(Callback):
    def __init__(self, anal_fn, n_traj=256, n_steps=100):
        super().__init__(); self.anal_fn=anal_fn; self.n_traj=n_traj; self.n_steps=n_steps
    def on_validation_batch_end(self, trainer, pl_module, outputs, batch, batch_idx):
        if batch_idx>0: return
        dim=pl_module.hparams.dim;device=pl_module.device;n=self.n_traj;dt=1.0/self.n_steps
        for beta,label in [(0,"base"),(1,"guided")]:
            x=torch.zeros(n,dim,device=device);all_x,all_t=[x],[torch.zeros(n,device=device)]
            drift_fn=partial(pl_module.drift,beta=beta)
            for st in torch.linspace(0,1,self.n_steps+1,device=device)[:-1]:
                tv=st.expand(n);dx=drift_fn(x,tv)*dt;db=sqrt(2*pl_module.a*dt)*torch.randn_like(x)
                x=x+dx+db;all_x.append(x);all_t.append(torch.full((n,),float(st)+dt,device=device))
            all_x=torch.cat(all_x);all_t=torch.cat(all_t)
            with torch.no_grad():
                vp=pl_module.value_module(all_x,all_t);va=self.anal_fn(all_x,all_t)
            err=vp-va
...
def train_offpolicy(vm, max_steps, run_name, version=0):
    ...
    model = OffPolicyValue(base_score_module=base_drift, reward_function=reward_fn,
        value_module=vm, dim=D, a=a, lr=LR, loss_type="quad", analytical_value_fn=anal_fn)
    traj_cb = TrajectoryEvalCallback(anal_fn)
    logger = CSVLogger(LOG_DIR, name=run_name, version=version)
    trainer = L.Trainer(max_steps=max_steps, val_check_interval=max(1,max_steps//60),
...
def train_alternating(vm, total_on_steps, run_name, chunk_size, ema_decay, version=1):
    """Alternate chunks of off-policy and on-policy training."""
    # Create the on-policy model (owns the EMA)
    ...
    traj_cb = TrajectoryEvalCallback(anal_fn)
    ckpt_cb = ModelCheckpoint(dirpath=f"{CKPT_DIR}/{run_name}", save_last=True,
        save_top_k=1, monitor="val_reward_mean", mode="max", filename="best")
    logger = CSVLogger(LOG_DIR, name=run_name, version=version)

    steps_done = 0
    n_chunks = total_on_steps // (2 * chunk_size)  # each cycle = chunk off + chunk on

    for cycle in range(n_chunks):
        # --- Off-policy chunk ---
        off_ds = InterpolatingNumpyDataset(generating_function=gmm_sample, a=a, batch_size=1024)
        off_loader = DataLoader(off_ds, batch_size=LOADER_BATCH_SIZE)

        # Wrap model as OffPolicyValue for this chunk
        # Instead, just train the OnPolicyValue with off-policy data.
        # We need to convert (x1, x, t) → (r(x1), x, t) format.
        # Simpler: create a small wrapper dataset.
        class OffPolicyAsOnPolicy:
            """Wraps off-policy dataset to yield (target, x, t) like on-policy."""
            def __init__(self, ds, reward):
                self.ds = ds; self.reward = reward
            def __iter__(self):
                for x1, x, t in self.ds:
                    target = self.reward(x1.unsqueeze(0)).squeeze(0)
                    yield target.unsqueeze(-1), x, t

        off_wrapped = OffPolicyAsOnPolicy(off_ds, reward_fn)
        off_loader = DataLoader(off_wrapped, batch_size=LOADER_BATCH_SIZE)

        off_trainer = L.Trainer(max_steps=chunk_size,
            val_check_interval=max(1, chunk_size // 2),
            callbacks=[traj_cb, ckpt_cb], logger=logger,
            enable_checkpointing=True, enable_progress_bar=False)
        off_trainer.fit(model, off_loader, val_dataloaders=val_loader)
        steps_done += chunk_size

        # --- On-policy chunk ---
        on_ds = OnPolicySMCDataset(dim=D, drift=base_drift, value=model.ema,
            smc_value=smc_reward, reward=reward_fn, device=DEVICE, a=a,
            batch_size=32, n_steps=100, mc_samples_per_step=10,
            sampling_method="ancestral_td_lambda", lambda_eff=0.0)
        on_loader = DataLoader(on_ds, batch_size=LOADER_BATCH_SIZE)

        on_trainer = L.Trainer(max_steps=chunk_size,
            val_check_interval=max(1, chunk_size // 2),
            callbacks=[traj_cb, ckpt_cb], logger=logger,
            enable_checkpointing=True, enable_progress_bar=False)
        on_trainer.fit(model, on_loader, val_dataloaders=val_loader)
        steps_done += chunk_size

        # Report progress
        val_df = pd.read_csv(f"{LOG_DIR}/{run_name}/version_{version}/metrics.csv")
        val_sub = val_df.dropna(subset=["val_reward_mean"])
        if len(val_sub) > 0:
            latest = val_sub["val_reward_mean"].iloc[-1]
            best = val_sub["val_reward_mean"].max()
            print(f"    Cycle {cycle+1}/{n_chunks}: steps={steps_done} latest={latest:.3f} best={best:.3f}")
...
def train_osb_blend(vm, max_steps, run_name, version=1):
    ...
    ds = OnPolicySMCDataset(dim=D, drift=base_drift, value=vm, smc_value=smc_reward,
        reward=reward_fn, device=DEVICE, a=a, batch_size=32, n_steps=100,
        mc_samples_per_step=10, sampling_method="one_step_bootstrap", lambda_eff=0.0)
    ...
    logger = CSVLogger(LOG_DIR, name=run_name, version=version)
    trainer = L.Trainer(max_steps=max_steps, val_check_interval=max(1,max_steps//60),
        callbacks=[ckpt_cb,traj_cb], logger=logger, enable_checkpointing=True, enable_progress_bar=True)
    trainer.fit(model, loader, val_dataloaders=val_loader)
    return vm


def train_pure_ema(vm, max_steps, run_name, ema_decay, version=1):
    model = OnPolicyValue(base_score_module=base_drift, value_module=vm,
        reward_function=reward_fn, dim=D, a=a, lr=LR,
        loss_type="quad", analytical_value_fn=anal_fn, ema_decay=ema_decay)
    ds = OnPolicySMCDataset(dim=D, drift=base_drift, value=model.ema, smc_value=smc_reward,
        reward=reward_fn, device=DEVICE, a=a, batch_size=32, n_steps=100,
        mc_samples_per_step=10, sampling_method="ancestral_td_lambda", lambda_eff=0.0)
    loader = DataLoader(ds, batch_size=LOADER_BATCH_SIZE)
    traj_cb = TrajectoryEvalCallback(anal_fn)
    ckpt_cb = ModelCheckpoint(dirpath=f"{CKPT_DIR}/{run_name}", save_last=True,
        save_top_k=1, monitor="val_reward_mean", mode="max", filename="best")
...
RUNS = [
    ...
    dict(name="alt50_ema999", method="alternating", chunk=50, ema_decay=0.999),
    dict(name="alt50_ema99", method="alternating", chunk=50, ema_decay=0.99),
    dict(name="alt100_ema999", method="alternating", chunk=100, ema_decay=0.999),
    dict(name="pure_ema999", method="pure_ema", ema_decay=0.999),
]

for cfg in RUNS:
    rn = cfg["name"]; method = cfg["method"]
    print(f"\n{'='*70}")
    print(f"  {rn}")
    print(f"{'='*70}")
    ...
    if method == "offpolicy":
        ...
        csv_check = f"{LOG_DIR}/{rn}/version_1/metrics.csv"
    ...
    if method == "offpolicy":
        train_offpolicy(vm, TOTAL_STEPS, rn)
        continue

    # Warm-start
    print(f"  Phase 1: Off-policy warm-start for {WS_STEPS} steps...")
    t0 = time.perf_counter()
    train_offpolicy(vm, WS_STEPS, rn, version=0)
    ...
    if method == "alternating":
        train_alternating(vm, ON_STEPS, rn, cfg["chunk"], cfg["ema_decay"])
    elif method == "osb_blend":
        train_osb_blend(vm, ON_STEPS, rn)
    elif method == "pure_ema":
        train_pure_ema(vm, ON_STEPS, rn, cfg["ema_decay"])
...
def load_combined(rn, method):
    dfs = []
    csv0 = f"{LOG_DIR}/{rn}/version_0/metrics.csv"
    if os.path.exists(csv0): dfs.append(pd.read_csv(csv0))
    if method != "offpolicy":
        csv1 = f"{LOG_DIR}/{rn}/version_1/metrics.csv"
        if os.path.exists(csv1):
            d = pd.read_csv(csv1).copy(); d["step"] = d["step"]+WS_STEPS; dfs.append(d)
    return pd.concat(dfs, ignore_index=True) if dfs else None

print(f"\n  {'Run':<22} {'Best':>8} {'Final':>8} {'Gap':>7} {'B-MAE':>7} {'G-MAE':>7}")
print(f"  {'-'*63}")
run_data = {}
for cfg in RUNS:
    rn = cfg["name"]
    ...
    print(f"  {rn:<22} {best:>8.3f} {final:>8.3f} {gap:>7.3f} {bf:>7.3f} {gf:>7.3f}")
    run_data[rn] = df

# Plot
fig, ax = plt.subplots(figsize=(14, 7))
ax.set_title("Alternating Training: Terminal Reward vs Steps", fontsize=13, fontweight="bold")
cmap=plt.cm.tab10
colors={cfg["name"]:cmap(i/len(RUNS)) for i,cfg in enumerate(RUNS)}
colors["offpolicy"]="black";colors["osb_blend"]="red"
for cfg in RUNS:
    rn=cfg["name"]
    if rn not in run_data: continue
    df=run_data[rn];val=df.dropna(subset=["val_reward_mean"])
    ls="--" if rn=="offpolicy" else (":" if "osb" in rn else "-")
    lw=2.5 if rn in ("offpolicy","osb_blend") else 1.5
    ax.plot(val["step"].values,val["val_reward_mean"].values,color=colors[rn],linestyle=ls,linewidth=lw,label=rn)
...
plt.tight_layout()
plt.savefig("experiments/misc/2026-05-06_alternating_training/alternating_exp_reward.png",dpi=150,bbox_inches="tight")
print(f"\nSaved: experiments/misc/2026-05-06_alternating_training/alternating_exp_reward.png")
plt.close()
