#!/usr/bin/env python3
"""5 seeds of SSMC with n_steps=10 random t-grid, no EMA in drift.

The trajectory time grid is [0, sorted U(0,1) × 9, 1] (10 SMC steps),
so each trajectory contributes 10 (x, t, target) samples at random
times. Compared against the existing 10-seed off-policy distribution
(experiments/misc/2026-05-28_offpolicy_seeds/offpolicy_seeds_results.json).

Validation rollout uses a fixed 100-step uniform grid so reward/MAE are
...
# ── Constants ──────────────────────────────────────────────────────────────
LOG_DIR = "lightning_logs/ssmc_random10_seeds"
CKPT_DIR = "checkpoints/ssmc_random10_seeds"
OFF_POLICY_RESULTS = "experiments/misc/2026-05-28_offpolicy_seeds/offpolicy_seeds_results.json"

TOTAL_STEPS = 1000
BS = 256
LR = 1e-3
N_STEPS_TRAIN = 10        # ← random t-grid: 10 SMC steps per trajectory
N_STEPS_VAL = 100         # ← val rollout / MAE callback (fair comparison)
MC_SAMPLES = 10
DS_BATCH = 40             # 40×10 = 400 samples/regen, matches prior n_steps=100
                          # throughput so regen overhead stays comparable.
N_SEEDS = 5
...
def run_one(seed):
    ...
    ds = OnPolicySMCDataset(
        ...
        a=a,
        batch_size=DS_BATCH,
        n_steps=N_STEPS_TRAIN,        # 10 SMC steps per trajectory
        mc_samples_per_step=MC_SAMPLES,
        sampling_method="single_seed_mc",
        random_t=True,                 # ← random sorted t-grid
        off_policy_frac=0.0,           # ← true SSMC (no off-policy splice)
        generating_function=gmm_sample,
    )
    loader = DataLoader(ds, batch_size=BS)
    tcb = TrajCB(anal_fn, ns=N_STEPS_VAL)
    ccb = ModelCheckpoint(
...
print(f"SSMC random_t  N_STEPS_TRAIN={N_STEPS_TRAIN}  N_STEPS_VAL={N_STEPS_VAL}  no-EMA in drift")
...
print(f"\n{'=' * 72}")
print(f"  SSMC random_t  N_STEPS_TRAIN={N_STEPS_TRAIN}  ({len(bests)} seeds)")
print(f"{'=' * 72}")
...
print(f"\n  Welch t-test  (off-policy vs SSMC random_t n_steps={N_STEPS_TRAIN})")
...
fig.suptitle(
    f"Off-policy (n={len(off_val_curves)})  vs  SSMC random_t  n_steps={N_STEPS_TRAIN} "
    f"(n={len(val_curves)})  —  1000 train steps",
...
ax.plot([], [], color=NEW_C, lw=2, label=f"SSMC random10 (n={len(val_curves)})")
...
ax.hist(bests, bins=5, color=NEW_C, alpha=0.65, edgecolor="white",
        label=f"SSMC random10 n={len(bests)}")
...
ax.hist(maes, bins=5, color=NEW_C, alpha=0.65, edgecolor="white",
        label=f"SSMC random10 n={len(maes)}")
...
plt.tight_layout()
out = "experiments/misc/2026-05-28_ssmc_sweeps/ssmc_random10_seeds.png"
plt.savefig(out, dpi=140, bbox_inches="tight")
print(f"\nSaved: {out}")

with open("experiments/misc/2026-05-28_ssmc_sweeps/ssmc_random10_seeds_results.json", "w") as f:
    json.dump(
        {
            "config": {
