#!/usr/bin/env python3
"""Sweep over k where smc_value(x, t) = k * ema_value(x, t).

Identical to ssmc_k_value_sweep.py except the SMC twist uses the EMA
*shadow* of the value module (smoothed) instead of the live network.
Drift/inference still use the live value (OnPolicyValueLive).
ema_decay = 0.99 → effective averaging window ~100 steps (10% of the
1000-step run), so the EMA tracks but smooths step-to-step noise.
"""
...
# ── Constants ──────────────────────────────────────────────────────────────
LOG_DIR = "lightning_logs/ssmc_k_ema_value_sweep"
...
def run_one(k, off_frac, seed):
    ...
    model = OnPolicyValueLive(
        ...
        analytical_value_fn=anal_fn, ema_decay=0.99,   # fast EMA: τ ≈ 100 steps
    )

    # EMA-shadow twist: smc_value(x, t) = k * ema_value(x, t).
    def smc_value_k(x, t):
        v = model.ema(x, t)
...
fig.suptitle(
    f"SSMC k-sweep, smc_value=k·V_ema(x,t) [decay=0.99],  mc={MC_SAMPLES}, "
    f"live-drift, include_t_zero=False  "
    f"(n={N_SEEDS} / cell vs off-policy n={len(off_bests)})",
...
plt.tight_layout()
out = "experiments/misc/2026-05-28_ssmc_sweeps/ssmc_k_ema_value_sweep.png"
...
summary = {
    "config": {
        ...
        "N_SEEDS": N_SEEDS,
        "include_t_zero": False, "use_ema_for_smc_value": True,
        "ema_decay": 0.99,
        "smc_value": "k * ema_value(x, t)",
...
with open("experiments/misc/2026-05-28_ssmc_sweeps/ssmc_k_ema_value_sweep_results.json", "w") as f:
