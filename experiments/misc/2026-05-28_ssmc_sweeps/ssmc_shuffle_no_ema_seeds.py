#!/usr/bin/env python3
"""5 seeds of SSMC k=0 with full-trajectory shuffled buffer and NO EMA in rollout.

Combines the two earlier experiments:
  - ssmc_shuffle_seeds.py:  full-trajectory buffer (2000 traj × 100 steps = 200K
                            samples), globally shuffled.
  - ssmc_1pt_no_ema.py:     subclass that flips drift's default to use_ema=False
                            so val_reward + traj_avg_mae_guided use the live
                            value_module (matching the off-policy benchmark).

Goal: characterise the SSMC k=0 distribution on a fair rollout protocol and
compare against:
  - off-policy 10-seed distribution (experiments/misc/2026-05-28_offpolicy_seeds/offpolicy_seeds_results.json)
  - SSMC k=0 shuffled WITH EMA (experiments/misc/2026-05-28_ssmc_sweeps/ssmc_shuffle_seeds_results.json)
"""
...
# Subclass: drift defaults to use_ema=False so rollout uses the live model.
...
# ── Constants ──────────────────────────────────────────────────────────────
LOG_DIR = "lightning_logs/ssmc_shuffle_no_ema_seeds"
CKPT_DIR = "checkpoints/ssmc_shuffle_no_ema_seeds"
OFF_POLICY_RESULTS = "experiments/misc/2026-05-28_offpolicy_seeds/offpolicy_seeds_results.json"
PRIOR_SHUF_EMA_RESULTS = "experiments/misc/2026-05-28_ssmc_sweeps/ssmc_shuffle_seeds_results.json"
...
N_STEPS = 100
MC_SAMPLES = 10
N_TRAJ_BIG = 2000        # 2000 × 100 = 200K samples per buffer (full traj)
N_SEEDS = 5
...
class TrajCB(Callback):
    ...
    def on_validation_batch_end(self, trainer, pl, outputs, batch, bi):
        ...
        for beta, label in [(0, "base"), (1, "guided")]:
            ...
            dfn = partial(pl.drift, beta=beta)   # subclass default = LIVE
...
# ── Buffer generator (full trajectories, k=0, then global shuffle) ─────────
@torch.no_grad()
...
print(f"Drift in rollout: LIVE (no EMA).  seeds = 0..{N_SEEDS - 1}\n")
...
print(f"\n  Prior SSMC k=0 shuffled WITH EMA (n={len(ema_bests)})")
...
print(f"\n  Welch t-test  (off-policy vs SSMC no-EMA)")
...
fig.suptitle(
    f"Off-policy ({len(off_val_curves)} seeds)  vs  SSMC k=0 shuffled NO-EMA "
    f"({len(val_curves)} seeds, full traj, N_TRAJ={N_TRAJ_BIG})  —  1000 steps",
    fontsize=12, fontweight="bold",
)

OFF_C = "#1f77b4"
EMA_C = "#d62728"   # prior-with-EMA (faint, for context)
NEW_C = "#2ca02c"
...
for val in val_curves:
    ax.plot(val["step"], val["val_reward_mean"], color=NEW_C, alpha=0.6, lw=1.4)
...
ax.plot([], [], color=NEW_C, lw=2, label=f"ssmc no-EMA (n={len(val_curves)})")
...
ax.hist(ema_bests, bins=5, color=EMA_C, alpha=0.45, edgecolor="white",
        label=f"ssmc shuf EMA n={len(ema_bests)}")
ax.hist(bests, bins=5, color=NEW_C, alpha=0.65, edgecolor="white",
        label=f"ssmc shuf no-EMA n={len(bests)}")
...
for mae in mae_curves:
    if len(mae) > 0:
        ax.plot(mae["step"], mae["traj_avg_mae_guided"], color=NEW_C, alpha=0.6, lw=1.4)
ax.plot([], [], color=OFF_C, lw=2, label=f"off-policy (n={len(off_mae_curves)})")
ax.plot([], [], color=NEW_C, lw=2, label=f"ssmc no-EMA (n={len(mae_curves)})")
ax.set_yscale("log")
ax.set_xlabel("training step")
...
ax.hist(ema_maes, bins=5, color=EMA_C, alpha=0.45, edgecolor="white",
        label=f"ssmc shuf EMA n={len(ema_maes)}")
ax.hist(maes, bins=5, color=NEW_C, alpha=0.65, edgecolor="white",
        label=f"ssmc shuf no-EMA n={len(maes)}")
...
plt.tight_layout()
out = "experiments/misc/2026-05-28_ssmc_sweeps/ssmc_shuffle_no_ema_seeds.png"
plt.savefig(out, dpi=140, bbox_inches="tight")
print(f"\nSaved: {out}")

with open("experiments/misc/2026-05-28_ssmc_sweeps/ssmc_shuffle_no_ema_seeds_results.json", "w") as f:
    json.dump(
        {
            "config": {
