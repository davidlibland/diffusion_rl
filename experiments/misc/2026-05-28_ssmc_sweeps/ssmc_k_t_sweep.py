#!/usr/bin/env python3
"""Geometric k-sweep for SSMC with smc_value = k * h(x) * t.

Pre-generates a shuffled buffer of 2000 trajectories per (k, seed) via
single_seed_mc(mc=10), then trains a value network for 1000 steps with the
live (no-EMA) drift and include_t_zero=False.  Compared against the existing
10-seed off-policy distribution.

k geometric grid (plus k=0 baseline): [0, 0.03, 0.1, 0.3].
"""
...
def make_smc_value(k):
    """smc_value(x, t) = k * h(x) * t."""
    def smc_value(x, t):
        return k * reward_fn(x) * t.reshape(-1)
    return smc_value
...
# ── Constants ──────────────────────────────────────────────────────────────
LOG_DIR = "lightning_logs/ssmc_k_t_sweep"
...
@torch.no_grad()
def generate_buffer(k, n_traj, chunk=400):
    smc_value_k = make_smc_value(k)
    xs, ts, ys = [], [], []
...
def run_one(k, seed):
    ...
    t0 = time.time()
    x_buf, t_buf, y_buf = generate_buffer(k, N_TRAJ)
...
results = {}  # k -> {"bests":[..], "maes":[..], "val_curves":[..], "mae_curves":[..]}
for k in K_VALUES:
    bests, maes, vcs, mcs = [], [], [], []
    for s in range(N_SEEDS):
        name = f"k{k:.4f}_seed{s:02d}"
    ...
    results[k] = dict(bests=np.array(bests), maes=np.array(maes),
...
print("\n" + "=" * 72)
print(f"{'k':>8}  {'n':>3}  {'best mean':>10}  {'best sd':>8}  {'MAE mean':>10}  {'MAE sd':>8}")
print("=" * 72)
for k in K_VALUES:
    r = results[k]
    if len(r["bests"]) == 0:
        continue
    print(f"{k:>8.3f}  {len(r['bests']):>3}  "
          f"{r['bests'].mean():>10.3f}  {r['bests'].std(ddof=1) if len(r['bests'])>1 else 0:>8.3f}  "
          f"{r['maes'].mean():>10.3f}  {r['maes'].std(ddof=1) if len(r['maes'])>1 else 0:>8.3f}")
print(f"{'off':>8}  {len(off_bests):>3}  "
      f"{off_bests.mean():>10.3f}  {off_bests.std(ddof=1):>8.3f}  "
      f"{off_maes.mean():>10.3f}  {off_maes.std(ddof=1):>8.3f}")
print("=" * 72)
...
fig.suptitle(
    f"SSMC k-sweep, smc_value = k·h(x)·t,  mc={MC_SAMPLES}, no-EMA, "
    f"include_t_zero=False  (n={N_SEEDS} seeds / k vs off-policy n={len(off_bests)})",
    fontsize=11, fontweight="bold",
)
OFF_C = "#888888"
cmap = plt.cm.viridis
ks = [k for k in K_VALUES if len(results[k]["val_curves"]) > 0]
...
for v in off_vcs:
    ax.plot(v["step"], v["val_reward_mean"], color=OFF_C, alpha=0.35, lw=1.0)
for k in ks:
    for v in results[k]["val_curves"]:
        ax.plot(v["step"], v["val_reward_mean"], color=colors[k], alpha=0.85, lw=1.4)
...
ax.set_xlabel("training step"); ax.set_ylabel("avg terminal reward")
...
for m in off_mcs:
    if len(m) > 0:
        ax.plot(m["step"], m["traj_avg_mae_guided"], color=OFF_C, alpha=0.35, lw=1.0)
for k in ks:
    for m in results[k]["mae_curves"]:
        if len(m) > 0:
            ax.plot(m["step"], m["traj_avg_mae_guided"], color=colors[k], alpha=0.85, lw=1.4)
...
plt.tight_layout()
out = "experiments/misc/2026-05-28_ssmc_sweeps/ssmc_k_t_sweep.png"
...
summary = {
    "config": {
        ...
        "N_STEPS": N_STEPS, "MC_SAMPLES": MC_SAMPLES,
        "N_TRAJ": N_TRAJ, "N_SEEDS": N_SEEDS,
        "include_t_zero": False, "use_ema": False,
        "smc_value": "k * h(x) * t",
        "k_values": K_VALUES, "E_OPT": E_OPT,
    },
    "by_k": {
        str(k): {
            "best_reward_values": results[k]["bests"].tolist(),
            "final_guided_mae_values": results[k]["maes"].tolist(),
        }
        for k in K_VALUES
    },
    "off_policy_baseline": {
...
with open("experiments/misc/2026-05-28_ssmc_sweeps/ssmc_k_t_sweep_results.json", "w") as f:
