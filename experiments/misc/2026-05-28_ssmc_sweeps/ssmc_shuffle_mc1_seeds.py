#!/usr/bin/env python3
"""5 seeds of SSMC k=0, mc_samples=1, full-trajectory shuffled buffer, no EMA.

With mc=1 and log_tau=0, the SSMC algorithm degenerates:
  - Single proposal per step → trivial resampling.
  - Per-step `log_mean_v` over N=1 = single value (no Jensen bias).
  - Telescoped target for every (x_i, t_i) in a trajectory = h(x_T).

So each trajectory contributes 100 (x, t) points all sharing one terminal
target h(x_T) — the cleanest possible apples-to-apples vs off-policy:
  Off-policy:        target = h(x_1)     (one fresh x_1 per training pair)
  SSMC mc=1 shuf:    target = h(x_T)     (one terminal x_T per trajectory,
                                          shared across its 100 (x_i, t_i))

Compares to the just-completed mc=10 no-EMA result to test whether the
~1.4× MAE gap was the log_mean_exp bias.
"""
...
class OnPolicyValueLive(OnPolicyValue):
    """drift defaults to use_ema=False so val rollout uses the live model."""
    def drift(self, x, t, beta=1, use_ema=False):
        return super().drift(x, t, beta=beta, use_ema=use_ema)


# ── Constants ──────────────────────────────────────────────────────────────
LOG_DIR = "lightning_logs/ssmc_shuffle_mc1_seeds"
CKPT_DIR = "checkpoints/ssmc_shuffle_mc1_seeds"
OFF_POLICY_RESULTS = "experiments/misc/2026-05-28_offpolicy_seeds/offpolicy_seeds_results.json"
PRIOR_MC10_NOEMA_RESULTS = "experiments/misc/2026-05-28_ssmc_sweeps/ssmc_shuffle_no_ema_seeds_results.json"

TOTAL_STEPS = 1000
BS = 256
LR = 1e-3
N_STEPS = 100
MC_SAMPLES = 1                # ← key change: no branching
...
# ── Trajectory MAE callback (uses live drift via subclass default) ─────────
...
# ── Buffer generator (full trajectories, mc=1, k=0, then shuffle) ──────────
@torch.no_grad()
def generate_buffer(n_traj, chunk=400):
    print(f"  Generating {n_traj} trajectories (mc={MC_SAMPLES}) on {DEVICE}...")
    t0 = time.time()
    xs, ts, ys = [], [], []
    for start in range(0, n_traj, chunk):
        ...
        all_x, all_t, all_tgt = single_seed_mc(
            ...
            n_steps=N_STEPS,
            device=DEVICE,
            include_t_zero=False,  # (x=0, t=0) is a degenerate point — see ablation
        )
        xs.append(all_x.cpu())
        ts.append(all_t.cpu())
...
print(f"mc_samples={MC_SAMPLES} (NO branching).  Drift in rollout: LIVE.  seeds=0..{N_SEEDS-1}\n")
...
bests, finals, maes = [], [], []
val_curves, mae_curves = [], []
for s in range(N_SEEDS):
    val, mae = load_curves(f"{LOG_DIR}/seed_{s:02d}/version_0/metrics.csv")
    if val is None or len(val) == 0:
        continue
    bests.append(float(val["val_reward_mean"].max()))
    finals.append(float(val["val_reward_mean"].iloc[-1]))
    maes.append(float(mae["traj_avg_mae_guided"].iloc[-1]) if len(mae) else float("nan"))
    val_curves.append(val)
    mae_curves.append(mae)
...
mc10_data = None
if os.path.exists(PRIOR_MC10_NOEMA_RESULTS):
    with open(PRIOR_MC10_NOEMA_RESULTS) as f:
        mc10_data = json.load(f)
    mc10_bests = np.array(mc10_data["best_reward"]["values"])
    mc10_maes = np.array(mc10_data["final_guided_mae"]["values"])

print(f"\n{'=' * 72}")
print(f"  SSMC k=0 SHUFFLED, mc=1, NO EMA  ({len(bests)} seeds)")
print(f"{'=' * 72}")
print(f"  best reward      mean={bests.mean():>7.3f}   sd={bests.std(ddof=1):>5.3f}   "
      f"min={bests.min():>7.3f}  max={bests.max():>7.3f}")
print(f"  final reward     mean={finals.mean():>7.3f}   sd={finals.std(ddof=1):>5.3f}")
print(f"  final guided MAE mean={maes.mean():>6.3f}   sd={maes.std(ddof=1):>5.3f}   "
      f"min={maes.min():>6.3f}  max={maes.max():>6.3f}")

print(f"\n  OFF-POLICY (n={len(off_bests)})")
...
if mc10_data is not None:
    print(f"\n  PRIOR SSMC k=0 shuffled, mc=10, no-EMA (n={len(mc10_bests)})")
    print(f"  best reward      mean={mc10_bests.mean():>7.3f}   sd={mc10_bests.std(ddof=1):>5.3f}")
    print(f"  final guided MAE mean={mc10_maes.mean():>6.3f}   sd={mc10_maes.std(ddof=1):>5.3f}")

from scipy import stats
t_best, p_best = stats.ttest_ind(bests, off_bests, equal_var=False)
t_mae, p_mae = stats.ttest_ind(maes, off_maes, equal_var=False)
print(f"\n  Welch t-test  (off-policy vs SSMC mc=1 no-EMA)")
print(f"    best reward:      t={t_best:+.2f}  p={p_best:.3f}")
print(f"    final guided MAE: t={t_mae:+.2f}  p={p_mae:.3f}")

if mc10_data is not None:
    t_b2, p_b2 = stats.ttest_ind(bests, mc10_bests, equal_var=False)
    t_m2, p_m2 = stats.ttest_ind(maes, mc10_maes, equal_var=False)
    print(f"\n  Welch t-test  (mc=10 no-EMA vs mc=1 no-EMA)")
    print(f"    best reward:      t={t_b2:+.2f}  p={p_b2:.3f}")
    print(f"    final guided MAE: t={t_m2:+.2f}  p={p_m2:.3f}")


# Plot
fig, axes = plt.subplots(2, 2, figsize=(14, 9))
fig.suptitle(
    f"Off-policy (n={len(off_val_curves)})  vs  SSMC k=0 shuf no-EMA  "
    f"[mc=10 (n={len(mc10_bests) if mc10_data else 0}) and mc=1 (n={len(val_curves)})]  —  1000 steps",
...
if mc10_data is not None:
    for s in range(mc10_data["config"]["n_seeds"]):
        v, _ = load_curves(f"lightning_logs/ssmc_shuffle_no_ema_seeds/seed_{s:02d}/version_0/metrics.csv")
        if v is not None:
            ax.plot(v["step"], v["val_reward_mean"], color=MC10_C, alpha=0.4, lw=1.0)
for val in val_curves:
    ax.plot(val["step"], val["val_reward_mean"], color=MC1_C, alpha=0.7, lw=1.4)
ax.axhline(E_OPT, color="black", ls=":", alpha=0.6, label=f"E_opt={E_OPT:.2f}")
ax.plot([], [], color=OFF_C, lw=2, label=f"off-policy (n={len(off_val_curves)})")
ax.plot([], [], color=MC10_C, lw=2, label="ssmc shuf no-EMA mc=10")
ax.plot([], [], color=MC1_C, lw=2, label=f"ssmc shuf no-EMA mc=1 (this run)")
...
if mc10_data is not None:
    ax.hist(mc10_bests, bins=5, color=MC10_C, alpha=0.5, edgecolor="white",
            label=f"ssmc mc=10 n={len(mc10_bests)}")
ax.hist(bests, bins=5, color=MC1_C, alpha=0.65, edgecolor="white",
        label=f"ssmc mc=1 n={len(bests)}")
...
if mc10_data is not None:
    for s in range(mc10_data["config"]["n_seeds"]):
        _, m = load_curves(f"lightning_logs/ssmc_shuffle_no_ema_seeds/seed_{s:02d}/version_0/metrics.csv")
        if m is not None and len(m) > 0:
            ax.plot(m["step"], m["traj_avg_mae_guided"], color=MC10_C, alpha=0.4, lw=1.0)
for mae in mae_curves:
    if len(mae) > 0:
        ax.plot(mae["step"], mae["traj_avg_mae_guided"], color=MC1_C, alpha=0.7, lw=1.4)
ax.plot([], [], color=OFF_C, lw=2, label=f"off-policy (n={len(off_mae_curves)})")
ax.plot([], [], color=MC10_C, lw=2, label="ssmc shuf no-EMA mc=10")
ax.plot([], [], color=MC1_C, lw=2, label=f"ssmc shuf no-EMA mc=1 (this run)")
ax.set_yscale("log")
ax.set_xlabel("training step")
...
if mc10_data is not None:
    ax.hist(mc10_maes, bins=5, color=MC10_C, alpha=0.5, edgecolor="white",
            label=f"ssmc mc=10 n={len(mc10_maes)}")
ax.hist(maes, bins=5, color=MC1_C, alpha=0.65, edgecolor="white",
        label=f"ssmc mc=1 n={len(maes)}")
...
plt.tight_layout()
out = "experiments/misc/2026-05-28_ssmc_sweeps/ssmc_shuffle_mc1_seeds.png"
plt.savefig(out, dpi=140, bbox_inches="tight")
print(f"\nSaved: {out}")

with open("experiments/misc/2026-05-28_ssmc_sweeps/ssmc_shuffle_mc1_seeds_results.json", "w") as f:
    json.dump(
        {
            "config": {
                "TOTAL_STEPS": TOTAL_STEPS, "BS": BS, "LR": LR,
                "MC_SAMPLES": MC_SAMPLES, "N_TRAJ_BIG": N_TRAJ_BIG,
                "use_ema": False, "n_seeds": len(bests), "E_OPT": E_OPT,
            },
            "best_reward": {"mean": float(bests.mean()),
                            "sd": float(bests.std(ddof=1)),
                            "values": bests.tolist()},
            "final_reward": {"mean": float(finals.mean()),
                             "sd": float(finals.std(ddof=1)),
                             "values": finals.tolist()},
            "final_guided_mae": {"mean": float(maes.mean()),
                                 "sd": float(maes.std(ddof=1)),
                                 "values": maes.tolist()},
            "vs_offpolicy_p_best": float(p_best),
            "vs_offpolicy_p_mae": float(p_mae),
        },
        f, indent=2,
    )
