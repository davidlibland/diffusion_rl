#!/usr/bin/env python3
"""Off-policy baseline vs single_seed_mc on moons (matched batch sizes).

Both runs use:
  - quadratic Bregman loss (loss_type="quad")
  - same DataLoader batch size (BS)
  - same lr, n_steps, a, EMA decay, max_steps

For single_seed_mc, log_tau is the reward h itself (independent of t).
"""
...
# ── GMM setup (matches mc_samples_ssmc_sweep) ──────────────────────────────
...
# ── Analytical (oracle) value function ─────────────────────────────────────
class AV(nn.Module):
...
# ── Drift, reward, sampler (matches mc_samples_ssmc_sweep) ─────────────────
...
# log_tau(x, t) = h(x), regardless of t — per user request.
def smc_log_tau(x, t):
    return reward_fn(x)
...
# ── Config ─────────────────────────────────────────────────────────────────
LOG_DIR = "lightning_logs/ssmc_vs_offpolicy"
CKPT_DIR = "checkpoints/ssmc_vs_offpolicy"

TOTAL_STEPS = 3000
BS = 256                # DataLoader batch — SAME for both on- and off-policy
LR = 1e-3
EMA_DECAY = 0.999
N_STEPS = 100           # SDE discretization
MC_SAMPLES = 10         # SMC proposals per step (single_seed_mc only)
OFF_POLICY_FRAC = 0.0   # pure single_seed_mc (no off-policy mixing)
DS_BATCH_ON = max(1, ceil(32 / MC_SAMPLES))   # internal trajectory batch

val_loader = DataLoader(TensorDataset(torch.zeros(1)), batch_size=1)


# ── Trajectory MAE callback (computes value error vs oracle) ──────────────
class TrajCB(Callback):
    """At each validation, integrate base + guided trajectories from x=0,
    then compute MAE between learned value and oracle along those paths.
    """
...
# ── Sweep configs (just two runs) ──────────────────────────────────────────
configs = [
    {"name": "offpolicy", "method": "offpolicy"},
    {"name": "single_seed_mc", "method": "single_seed_mc"},
]

print(f"E_OPT = {E_OPT:.4f}")
print(f"Device: {DEVICE}")
print(f"BS={BS}  LR={LR}  TOTAL_STEPS={TOTAL_STEPS}  N_STEPS={N_STEPS}")
print(f"single_seed_mc: mc={MC_SAMPLES}  off_policy_frac={OFF_POLICY_FRAC}  log_tau=h(x)")
print()
...
for i, cfg in enumerate(configs):
    ...
    if os.path.exists(csv_path):
        ...
        if len(val) > 0 and int(val["step"].max()) >= TOTAL_STEPS - 1:
            print(f"[{i + 1}/{len(configs)}] {name}: already complete, skipping.")
            continue
    ...
    print(f"\n{'=' * 70}")
    print(f"  [{i + 1}/{len(configs)}] {name}  method={cfg['method']}")
    print(f"{'=' * 70}")
    ...
    if cfg["method"] == "offpolicy":
        ...
        ds = OnPolicySMCDataset(
            ...
            drift=base_drift,
            value=model.ema,
            smc_value=smc_log_tau,    # log_tau(x, t) = h(x)
            reward=reward_fn,
            device=DEVICE,
            a=a,
            batch_size=DS_BATCH_ON,   # internal trajectory batch
            n_steps=N_STEPS,
    ...
    # SAME DataLoader batch size for both branches.
...
def load_metrics(name):
    p = f"{LOG_DIR}/{name}/version_0/metrics.csv"
...
summary = {}
for cfg in configs:
    df = load_metrics(cfg["name"])
    if df is None:
        print(f"  {cfg['name']}: no metrics found")
        continue
    val = df.dropna(subset=["val_reward_mean"])
    gm = df.dropna(subset=["traj_avg_mae_guided"])
    bm = df.dropna(subset=["traj_avg_mae_base"])
    summary[cfg["name"]] = {
        "best_reward": float(val["val_reward_mean"].max()) if len(val) else float("nan"),
        "final_reward": float(val["val_reward_mean"].iloc[-1]) if len(val) else float("nan"),
        "final_guided_mae": float(gm["traj_avg_mae_guided"].iloc[-1]) if len(gm) else float("nan"),
        "final_base_mae": float(bm["traj_avg_mae_base"].iloc[-1]) if len(bm) else float("nan"),
    }
    print(
        f"  {cfg['name']:<20} best={summary[cfg['name']]['best_reward']:>7.3f}"
        f"  final={summary[cfg['name']]['final_reward']:>7.3f}"
        f"  guided_MAE={summary[cfg['name']]['final_guided_mae']:>6.3f}"
        f"  base_MAE={summary[cfg['name']]['final_base_mae']:>6.3f}"
    )

# Plot: reward and value error vs training step.
fig, axes = plt.subplots(1, 2, figsize=(14, 5))
fig.suptitle(
    "Off-policy baseline vs single_seed_mc (log_tau = h)  —  matched batch size",
    fontsize=13,
    fontweight="bold",
)

styles = {
    "offpolicy": {"color": "#1f77b4", "ls": "--", "label": "off-policy (baseline)"},
    "single_seed_mc": {"color": "#d62728", "ls": "-", "label": "single_seed_mc (log_tau=h)"},
}

# Left: terminal reward vs step
ax = axes[0]
...
for cfg in configs:
    ...
    val = df.dropna(subset=["val_reward_mean"])
    s = styles[cfg["name"]]
    ax.plot(val["step"], val["val_reward_mean"], lw=2, **s)
...
# Right: value-function error (MAE vs oracle) on guided trajectories
ax = axes[1]
...
for cfg in configs:
    ...
    s = styles[cfg["name"]]
    ax.plot(sub["step"], sub["traj_avg_mae_guided"], lw=2, **s)
ax.set_xlabel("training step")
ax.set_ylabel("MAE  (|V_θ - V*|, mean over trajectory)")
ax.set_yscale("log")
ax.legend()
ax.grid(True, alpha=0.3, which="both")

plt.tight_layout()
out = "experiments/misc/2026-05-28_ssmc_sweeps/ssmc_vs_offpolicy.png"
plt.savefig(out, dpi=150, bbox_inches="tight")
print(f"\nSaved plot: {out}")

# Save summary JSON
with open("experiments/misc/2026-05-28_ssmc_sweeps/ssmc_vs_offpolicy_results.json", "w") as f:
    json.dump(
        {
            "config": {
