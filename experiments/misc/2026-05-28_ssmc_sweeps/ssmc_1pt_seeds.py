#!/usr/bin/env python3
"""5 seeds of SSMC k=0 with 256K trajectories, 1 sample per trajectory.

This matches off-policy's unique-sample budget over 1000 training steps:
  256_000 trajectories × 1 sample/traj = 256K independent samples
  ~= 1000 steps × 256 batch (one epoch).

If the residual gap to off-policy was just sample efficiency, this
configuration should put SSMC inside the off-policy distribution.

After all 5 seeds finish, this script merges the off-policy seeds results
from `experiments/misc/2026-05-28_offpolicy_seeds/offpolicy_seeds_results.json` and produces a side-by-side
distribution plot.
"""
...
# ── Constants ──────────────────────────────────────────────────────────────
LOG_DIR = "lightning_logs/ssmc_1pt_seeds"
CKPT_DIR = "checkpoints/ssmc_1pt_seeds"
OFF_POLICY_RESULTS = "experiments/misc/2026-05-28_offpolicy_seeds/offpolicy_seeds_results.json"
PRIOR_SHUF_RESULTS = "experiments/misc/2026-05-28_ssmc_sweeps/ssmc_shuffle_seeds_results.json"
...
N_STEPS = 100
MC_SAMPLES = 10
N_TRAJ_BIG = 256_000        # matches off-policy's unique-sample budget
N_SEEDS = 5
...
def generate_buffer_one_per_traj(n_traj, chunk=800):
    """Generate `n_traj` SSMC trajectories with log_tau=0; keep 1 random sample per traj."""
...
shuf_data = None
shuf_val_curves, shuf_mae_curves = [], []
if os.path.exists(PRIOR_SHUF_RESULTS):
    with open(PRIOR_SHUF_RESULTS) as f:
        shuf_data = json.load(f)
    for s in range(shuf_data["n_seeds"]):
        val, mae = load_curves(f"lightning_logs/ssmc_shuffle_seeds/seed_{s:02d}/version_0/metrics.csv")
        if val is not None:
            shuf_val_curves.append(val)
            shuf_mae_curves.append(mae)


print(f"\n{'=' * 72}")
print(f"  SSMC k=0  1-PER-TRAJ ({len(ssmc_bests)} seeds, N_TRAJ={N_TRAJ_BIG})")
print(f"{'=' * 72}")
...
if off_data is not None:
    ...
    print(f"\n  OFF-POLICY ({off_data['n_seeds']} seeds)")
...
if shuf_data is not None:
    shuf_bests = np.array(shuf_data["best_reward"]["values"])
    shuf_maes = np.array(shuf_data["final_guided_mae"]["values"])
    print(f"\n  SSMC k=0 SHUFFLED ({shuf_data['n_seeds']} seeds, N_TRAJ=2K full-traj)")
    print(f"  best reward      mean={shuf_bests.mean():>7.3f}   sd={shuf_bests.std(ddof=1):>5.3f}")
    print(f"  final guided MAE mean={shuf_maes.mean():>6.3f}   sd={shuf_maes.std(ddof=1):>5.3f}")
...
if off_data is not None:
    ...
    print(f"\n  Welch t-test  (off-policy vs SSMC 1-per-traj)")
...
fig.suptitle(
    f"Off-policy ({len(off_val_curves)} seeds)  vs  SSMC k=0 1-per-traj  "
    f"({len(ssmc_val_curves)} seeds, N_TRAJ={N_TRAJ_BIG})  —  1000 steps",
...
for val in shuf_val_curves:
    ax.plot(val["step"], val["val_reward_mean"], color=SHUF_C, alpha=0.3, lw=1.0)
for val in ssmc_val_curves:
    ax.plot(val["step"], val["val_reward_mean"], color=SSMC_C, alpha=0.6, lw=1.4)
ax.axhline(E_OPT, color="black", ls=":", alpha=0.6, label=f"E_opt={E_OPT:.2f}")
ax.plot([], [], color=OFF_C, lw=2, label="off-policy seeds")
ax.plot([], [], color=SHUF_C, lw=2, label="ssmc k=0 shuffle (2K traj)")
ax.plot([], [], color=SSMC_C, lw=2, label=f"ssmc k=0 1-per-traj ({N_TRAJ_BIG} traj)")
...
if shuf_data is not None:
    ax.hist(shuf_bests, bins=5, color=SHUF_C, alpha=0.55, edgecolor="white",
            label=f"ssmc shuf n={len(shuf_bests)}")
ax.hist(ssmc_bests, bins=5, color=SSMC_C, alpha=0.55, edgecolor="white",
        label=f"ssmc 1pt n={len(ssmc_bests)}")
...
for mae in shuf_mae_curves:
    if len(mae) > 0:
        ax.plot(mae["step"], mae["traj_avg_mae_guided"], color=SHUF_C, alpha=0.3, lw=1.0)
for mae in ssmc_mae_curves:
    if len(mae) > 0:
        ax.plot(mae["step"], mae["traj_avg_mae_guided"], color=SSMC_C, alpha=0.6, lw=1.4)
ax.plot([], [], color=OFF_C, lw=2, label="off-policy seeds")
ax.plot([], [], color=SHUF_C, lw=2, label="ssmc k=0 shuffle (2K traj)")
ax.plot([], [], color=SSMC_C, lw=2, label=f"ssmc k=0 1-per-traj ({N_TRAJ_BIG} traj)")
ax.set_yscale("log")
...
if shuf_data is not None:
    ax.hist(shuf_maes, bins=5, color=SHUF_C, alpha=0.55, edgecolor="white",
            label=f"ssmc shuf n={len(shuf_maes)}")
ax.hist(ssmc_maes, bins=5, color=SSMC_C, alpha=0.55, edgecolor="white",
        label=f"ssmc 1pt n={len(ssmc_maes)}")
...
plt.tight_layout()
out = "experiments/misc/2026-05-28_ssmc_sweeps/ssmc_1pt_seeds.png"
plt.savefig(out, dpi=140, bbox_inches="tight")
print(f"\nSaved: {out}")

with open("experiments/misc/2026-05-28_ssmc_sweeps/ssmc_1pt_seeds_results.json", "w") as f:
    json.dump(
        {
            "config": {
                "TOTAL_STEPS": TOTAL_STEPS, "BS": BS, "LR": LR,
                "MC_SAMPLES": MC_SAMPLES, "N_TRAJ_BIG": N_TRAJ_BIG,
                "E_OPT": E_OPT, "n_seeds": len(ssmc_bests),
            },
