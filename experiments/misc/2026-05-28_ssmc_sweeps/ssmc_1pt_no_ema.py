...
with open("experiments/misc/2026-05-28_offpolicy_seeds/offpolicy_seeds_results.json") as f:
    off_data = json.load(f)
off_bests = np.array(off_data["best_reward"]["values"])
off_maes = np.array(off_data["final_guided_mae"]["values"])

with open("experiments/misc/2026-05-28_ssmc_sweeps/ssmc_1pt_seeds_results.json") as f:
...
plt.tight_layout()
out = "experiments/misc/2026-05-28_ssmc_sweeps/ssmc_1pt_no_ema.png"
plt.savefig(out, dpi=140, bbox_inches="tight")
print(f"\nSaved: {out}")

with open("experiments/misc/2026-05-28_ssmc_sweeps/ssmc_1pt_no_ema_results.json", "w") as f:
    json.dump(
        {
            "config": {
