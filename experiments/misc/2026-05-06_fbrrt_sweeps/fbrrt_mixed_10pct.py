"""
FBRRT with 50% warm-start → 10% off-policy mix.

Steps 0-3000:    off_policy_frac=0.5 (50/50 warm-start)
Steps 3000-6000: off_policy_frac=0.1 (90% FBRRT, 10% off-policy anchor)

Same best config: FBRRT λ=0, ent_λ=2, gd=1e-6, EMA=0.999, LR=1e-3, branch=4
"""
...
TOTAL_STEPS = 6000; WS_STEPS = 3000; LOADER_BATCH_SIZE = 256
...
class FracScheduleCallback(Callback):
    def __init__(self, dataset, switch_step, new_frac):
        super().__init__()
        self.dataset = dataset; self.switch_step = switch_step
        self.new_frac = new_frac; self.switched = False
...
# --- Run ---
name = "fbrrt_ent2_gd1e6_ws50_on10"
print(f"\n{'='*70}")
print(f"  {name}")
print(f"  Steps 0-{WS_STEPS}: off_policy_frac=0.5 (50% mix warm-start)")
print(f"  Steps {WS_STEPS}-{TOTAL_STEPS}: off_policy_frac=0.1 (90% FBRRT)")
print(f"{'='*70}")
...
ds = OnPolicySMCDataset(
    ...
    off_policy_frac=0.5,  # start with 50% mix
...
frac_cb = FracScheduleCallback(ds, switch_step=WS_STEPS, new_frac=0.1)
...
# --- Comparison plot ---
runs = {}
runs["FBRRT ws50→on10"] = df_new

# Load previous runs
csv = "lightning_logs/fbrrt_mixed/fbrrt_ent2_gd1e6_mixed50/version_0/metrics.csv"
if os.path.exists(csv):
    runs["FBRRT ws100→on50"] = pd.read_csv(csv)

# FBRRT pure on-policy
dfs = []
for v in [0, 1]:
...
for log in ["lightning_logs/fbrrt_sweep/osb_blend_ws3k", "lightning_logs/mixed_final/osb_blend"]:
    ...
    for v in [0, 1]:
        ...
        if os.path.exists(csv):
            ...
            dfs.append(d)
    if dfs:
        runs["OSB+Blend"] = pd.concat(dfs, ignore_index=True); break

print(f"\nLoaded {len(runs)} runs for comparison")

styles = {
    "Off-Policy":           dict(color="black", ls="--", lw=2.5),
    "OSB+Blend":            dict(color="red", ls=":", lw=2.5),
    "FBRRT Pure On-Policy": dict(color="gray", ls="-", lw=1.5),
    "FBRRT ws100→on50":     dict(color="#1a73e8", ls="-", lw=2.0),
    "FBRRT ws50→on10":      dict(color="#e8771a", ls="-", lw=2.5),
}

fig, axes = plt.subplots(1, 2, figsize=(16, 6))
fig.suptitle("FBRRT Mixed Comparison", fontsize=14, fontweight="bold")

for name_r, df_r in runs.items():
    s = styles.get(name_r, dict(color="blue", ls="-", lw=1.5))
    val_r = df_r.dropna(subset=["val_reward_mean"])
    axes[0].plot(val_r["step"].values, val_r["val_reward_mean"].values, label=name_r, **s)
    sub = df_r.dropna(subset=["traj_avg_mae_guided"])
    if len(sub) > 0:
        axes[1].plot(sub["step"].values, sub["traj_avg_mae_guided"].values, label=name_r, **s)

axes[0].axhline(E_OPT, color="red", linestyle=":", linewidth=1, alpha=0.4, label=f"E_opt={E_OPT:.3f}")
axes[0].axvline(3000, color="gray", linestyle=":", alpha=0.3)
...
axes[1].axvline(3000, color="gray", linestyle=":", alpha=0.3)
axes[1].set_xlabel("Steps"); axes[1].set_ylabel("Avg MAE")
axes[1].set_title("V Error on Guided Trajectories (β=1)")
axes[1].legend(fontsize=7); axes[1].grid(True, alpha=0.3); axes[1].set_yscale("log")

plt.tight_layout()
plt.savefig("experiments/misc/2026-05-06_fbrrt_sweeps/fbrrt_mixed_comparison_v2.png", dpi=150, bbox_inches="tight")
print(f"Saved: experiments/misc/2026-05-06_fbrrt_sweeps/fbrrt_mixed_comparison_v2.png")
plt.close()
print("Done.")
