"""
FBRRT with mixed off-policy/on-policy using OnPolicySMCDataset.off_policy_frac.

Single model, single optimizer throughout.
  - Steps 0-3000:    off_policy_frac=1.0 (pure off-policy warm-start)
  - Steps 3000-6000: off_policy_frac=0.5 (50% on-policy FBRRT, 50% off-policy)

Best FBRRT config: entropy_lambda=2, grad_decay=1e-6, EMA=0.999, LR=1e-3
"""
...
# --- Run ---
name = "fbrrt_ent2_gd1e6_mixed50"
print(f"\n{'='*70}")
print(f"  {name}")
print(f"  FBRRT ent_λ=2, gd=1e-6, EMA=0.999, LR=1e-3")
print(f"  Steps 0-{WS_STEPS}: off_policy_frac=1.0")
print(f"  Steps {WS_STEPS}-{TOTAL_STEPS}: off_policy_frac=0.5")
print(f"{'='*70}")
...
frac_cb = FracScheduleCallback(ds, switch_step=WS_STEPS, new_frac=0.5)
...
fig.suptitle(f"FBRRT Mixed (ent_λ=2, gd=1e-6, 50% off-policy after ws)", fontsize=13, fontweight="bold")

val = df.dropna(subset=["val_reward_mean"])
axes[0].plot(val["step"].values, val["val_reward_mean"].values, "b-", linewidth=1.5, label=name)
axes[0].axhline(E_OPT, color="red", linestyle=":", linewidth=1.5, alpha=0.5, label=f"E_opt={E_OPT:.3f}")
axes[0].axvline(WS_STEPS, color="gray", linestyle=":", alpha=0.5, label="ws→mixed")
...
for metric, label, color in [("traj_avg_mae_base", "base (β=0)", "blue"),
                               ("traj_avg_mae_guided", "guided (β=1)", "green")]:
    sub = df.dropna(subset=[metric])
    if len(sub) > 0:
        axes[1].plot(sub["step"].values, sub[metric].values, color=color, linewidth=1.5, label=label)
axes[1].axvline(WS_STEPS, color="gray", linestyle=":", alpha=0.5)
axes[1].set_xlabel("Steps"); axes[1].set_ylabel("Avg MAE")
axes[1].set_title("V Error Along Trajectories"); axes[1].legend(fontsize=8)
axes[1].grid(True, alpha=0.3); axes[1].set_yscale("log")

plt.tight_layout()
plt.savefig("experiments/misc/2026-05-06_fbrrt_sweeps/fbrrt_mixed_run.png", dpi=150, bbox_inches="tight")
print(f"Saved: experiments/misc/2026-05-06_fbrrt_sweeps/fbrrt_mixed_run.png")
plt.close()
print("Done.")
