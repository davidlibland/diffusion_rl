"""
FBRRT with titrated off-policy fraction and fbrrt_alpha.

Schedule (6000 total steps):
  Steps 0-1500 (25%):      off_policy_frac=1.0, fbrrt_alpha=0.5 (pure off-policy warm-start)
  Steps 1500-6000 (75%):   Linear titration:
                            off_policy_frac: 1.0 → 0.1
                            fbrrt_alpha: 0.5 → 1.0

Best config: FBRRT λ=0, ent_λ=2, gd=1e-6, EMA=0.999, LR=1e-3, branch=4
"""
...
LOG_DIR = "lightning_logs/fbrrt_mixed"; CKPT_DIR = "checkpoints/fbrrt_mixed"
...
class TitrateCallback(Callback):
    """Linearly titrate off_policy_frac and fbrrt_alpha after warm-start."""
    def __init__(self, dataset, ws_step, total_steps,
                 frac_start, frac_end, alpha_start, alpha_end):
        super().__init__()
    ...
    def on_train_batch_start(self, trainer, pl_module, batch, batch_idx):
        ...
        if step < self.ws_step:
            return  # still in warm-start
        if not self.started:
            print(f"  [step {step}] Titration started")
            self.started = True
        # Linear interpolation from ws_step to total_steps
        progress = min(1.0, (step - self.ws_step) / (self.total_steps - self.ws_step))
        frac = self.frac_start + (self.frac_end - self.frac_start) * progress
        alpha = self.alpha_start + (self.alpha_end - self.alpha_start) * progress
        self.dataset.off_policy_frac = frac
        self.dataset.fbrrt_alpha = alpha
...
print(f"\n{'='*70}")
print(f"  {name}")
print(f"  Steps 0-{WS_STEPS}: off_policy_frac=1.0 (pure off-policy)")
print(f"  Steps {WS_STEPS}-{TOTAL_STEPS}: titrate frac 1.0→0.1, alpha 0.5→1.0")
print(f"{'='*70}")
...
ds = OnPolicySMCDataset(
    ...
    branch=4, entropy_lambda=2.0, fbrrt_alpha=0.5,
...
titrate_cb = TitrateCallback(
    ds, ws_step=WS_STEPS, total_steps=TOTAL_STEPS,
    frac_start=1.0, frac_end=0.1,
...
trainer = L.Trainer(
    ...
    callbacks=[ckpt_cb, traj_cb, titrate_cb],
    logger=logger, enable_checkpointing=True, enable_progress_bar=True,
)
...
val = df_new.dropna(subset=["val_reward_mean"])
best = val["val_reward_mean"].max(); final = val["val_reward_mean"].iloc[-1]
bm = df_new.dropna(subset=["traj_avg_mae_base"]); gm = df_new.dropna(subset=["traj_avg_mae_guided"])
...
print(f"\nResult: best={best:.3f} final={final:.3f} B-MAE={bf:.3f} G-MAE={gf:.3f} stable={stable}")

# --- Comparison plot ---
runs = {}
runs["FBRRT Titrate"] = df_new

csv = "lightning_logs/fbrrt_mixed/fbrrt_ent2_gd1e6_mixed50/version_0/metrics.csv"
if os.path.exists(csv): runs["FBRRT ws100→on50"] = pd.read_csv(csv)

csv = "lightning_logs/fbrrt_mixed/fbrrt_ent2_gd1e6_ws50_on10/version_0/metrics.csv"
if os.path.exists(csv): runs["FBRRT ws50→on10"] = pd.read_csv(csv)

dfs = []
for v in [0, 1]:
    csv = f"lightning_logs/fbrrt_gd_sweep/fbrrt_ent2_gd1e-06/version_{v}/metrics.csv"
    if os.path.exists(csv):
        d = pd.read_csv(csv)
        if v == 1: d = d.copy(); d["step"] = d["step"] + 3000
        dfs.append(d)
if dfs: runs["FBRRT Pure On-Policy"] = pd.concat(dfs, ignore_index=True)

for log in ["lightning_logs/fbrrt_sweep/offpolicy", "lightning_logs/mixed_final/offpolicy"]:
    csv = f"{log}/version_0/metrics.csv"
    if os.path.exists(csv): runs["Off-Policy"] = pd.read_csv(csv); break

for log in ["lightning_logs/fbrrt_sweep/osb_blend_ws3k", "lightning_logs/mixed_final/osb_blend"]:
    dfs = []
    for v in [0, 1]:
        csv = f"{log}/version_{v}/metrics.csv"
        if os.path.exists(csv):
            d = pd.read_csv(csv)
            if v == 1: d = d.copy(); d["step"] = d["step"] + 3000
            dfs.append(d)
    if dfs: runs["OSB+Blend"] = pd.concat(dfs, ignore_index=True); break

styles = {
    "Off-Policy":           dict(color="black", ls="--", lw=2.5),
    "OSB+Blend":            dict(color="red", ls=":", lw=2.5),
    "FBRRT Pure On-Policy": dict(color="gray", ls="-", lw=1.5),
    "FBRRT ws100→on50":     dict(color="#1a73e8", ls="-", lw=1.5),
    "FBRRT ws50→on10":      dict(color="#e8771a", ls="-", lw=1.5),
    "FBRRT Titrate":        dict(color="#2ecc71", ls="-", lw=2.5),
}

fig, axes = plt.subplots(1, 2, figsize=(16, 6))
fig.suptitle("FBRRT Titration: frac 1.0→0.1, alpha 0.5→1.0", fontsize=14, fontweight="bold")

for nm, df_r in runs.items():
    s = styles.get(nm, dict(color="blue", ls="-", lw=1.5))
    val_r = df_r.dropna(subset=["val_reward_mean"])
    axes[0].plot(val_r["step"].values, val_r["val_reward_mean"].values, label=nm, **s)
    sub = df_r.dropna(subset=["traj_avg_mae_guided"])
    if len(sub) > 0:
        axes[1].plot(sub["step"].values, sub["traj_avg_mae_guided"].values, label=nm, **s)

axes[0].axhline(E_OPT, color="red", linestyle=":", linewidth=1, alpha=0.4, label=f"E_opt={E_OPT:.3f}")
axes[0].axvline(WS_STEPS, color="gray", linestyle=":", alpha=0.3, label=f"titration start ({WS_STEPS})")
axes[0].set_xlabel("Steps"); axes[0].set_ylabel("Avg Terminal Reward")
axes[0].set_title("Terminal Reward"); axes[0].legend(fontsize=6); axes[0].grid(True, alpha=0.3)

axes[1].axvline(WS_STEPS, color="gray", linestyle=":", alpha=0.3)
axes[1].set_xlabel("Steps"); axes[1].set_ylabel("Avg MAE")
axes[1].set_title("V Error on Guided Trajectories (β=1)")
axes[1].legend(fontsize=6); axes[1].grid(True, alpha=0.3); axes[1].set_yscale("log")

plt.tight_layout()
plt.savefig("experiments/misc/2026-05-06_fbrrt_sweeps/fbrrt_titrate.png", dpi=150, bbox_inches="tight")
print(f"Saved: experiments/misc/2026-05-06_fbrrt_sweeps/fbrrt_titrate.png")
plt.close()
print("Done.")
