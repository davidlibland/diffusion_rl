"""
Simple mixed training: warm-start off-policy, then on-policy with EMA + lower LR.

The key insight from prior experiments: on-policy with EMA reaches good peaks
but crashes. Lower LR may prevent the crash by slowing parameter updates.

Configs:
  - offpolicy (baseline)
  - osb_blend ws=3000 (previous best)
  - ATD(λ=0) + EMA 0.999 + smc=reward, LR=3e-3 (same as before, for ref)
  - ATD(λ=0) + EMA 0.999 + smc=reward, LR=1e-3
  - ATD(λ=0) + EMA 0.999 + smc=reward, LR=3e-4
  - ATD(λ=0) + EMA 0.9999 + smc=reward, LR=3e-3
"""
...
LOG_DIR="lightning_logs/mixed_simple";CKPT_DIR="checkpoints/mixed_simple"
...
def train_onpolicy_ema(vm,max_steps,run_name,lr,ema_decay,version=1):
    ...
    loader=DataLoader(ds,batch_size=LOADER_BATCH_SIZE)
    traj_cb=TrajectoryEvalCallback(anal_fn)
    ckpt_cb=ModelCheckpoint(dirpath=f"{CKPT_DIR}/{run_name}",save_last=True,save_top_k=1,
...
RUNS=[
    ...
    dict(name="ema999_lr3e3",method="ema",lr=3e-3,ema_decay=0.999),
    dict(name="ema999_lr1e3",method="ema",lr=1e-3,ema_decay=0.999),
    dict(name="ema999_lr3e4",method="ema",lr=3e-4,ema_decay=0.999),
    dict(name="ema9999_lr3e3",method="ema",lr=3e-3,ema_decay=0.9999),
]

for cfg in RUNS:
    rn=cfg["name"];method=cfg["method"]
    print(f"\n{'='*70}\n  {rn}\n{'='*70}")
    if method=="offpolicy":csv_check=f"{LOG_DIR}/{rn}/version_0/metrics.csv";expected=TOTAL_STEPS
    else:csv_check=f"{LOG_DIR}/{rn}/version_1/metrics.csv";expected=ON_STEPS
    if os.path.exists(csv_check):
        df=pd.read_csv(csv_check);val=df.dropna(subset=["val_reward_mean"])
        if len(val)>0 and val["step"].max()>=expected-1:print("  Already complete.");continue
    for v in range(3):
        p=f"{LOG_DIR}/{rn}/version_{v}"
        if os.path.exists(p):shutil.rmtree(p)
    vm=ValueNetwork(D,bias=bias_val)
    if method=="offpolicy":
        train_offpolicy(vm,TOTAL_STEPS,rn);continue
    print(f"  Phase 1: Off-policy for {WS_STEPS} steps...")
    t0=time.perf_counter();train_offpolicy(vm,WS_STEPS,rn,version=0)
    print(f"  Phase 1 done: {(time.perf_counter()-t0)/60:.1f} min")
    print(f"  Phase 2: {method} for {ON_STEPS} steps...")
    t0=time.perf_counter()
    if method=="ema":train_onpolicy_ema(vm,ON_STEPS,rn,cfg["lr"],cfg["ema_decay"])
    elif method=="osb_blend":train_osb_blend(vm,ON_STEPS,rn)
...
def load_combined(rn,method):
    ...
    if method!="offpolicy":
        ...
        if os.path.exists(csv1):d=pd.read_csv(csv1).copy();d["step"]=d["step"]+WS_STEPS;dfs.append(d)
...
run_data={}
for cfg in RUNS:
    rn=cfg["name"];df=load_combined(rn,cfg["method"])
...
ax.set_title("LR + EMA Sweep: Terminal Reward vs Steps",fontsize=13,fontweight="bold")
cmap=plt.cm.tab10;colors={cfg["name"]:cmap(i/len(RUNS)) for i,cfg in enumerate(RUNS)}
...
plt.tight_layout();plt.savefig("experiments/misc/2026-05-06_mixed_training/mixed_simple_reward.png",dpi=150,bbox_inches="tight")
print(f"\nSaved: experiments/misc/2026-05-06_mixed_training/mixed_simple_reward.png");plt.close()
print(f"\nDone.")
