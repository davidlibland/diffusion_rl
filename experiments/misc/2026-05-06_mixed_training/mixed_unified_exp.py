"""
Unified mixed training using OnPolicySMCDataset.off_policy_frac.

Single model + single optimizer throughout. The off_policy_frac parameter
controls the mix:
  - Phase 1 (warm-start): off_policy_frac=1.0 (pure off-policy)
  - Phase 2 (on-policy):  off_policy_frac=0.0, 0.25, or 0.5

Since we use one OnPolicyValue model for both phases, the optimizer state
(Adam momentum) is preserved across the transition.

Configs:
  - pure_offpolicy (frac=1.0 for all 6000 steps)
  - osb_blend ws=3000 (previous best, for reference)
  - frac0_ema999 (ws=3000 then pure on-policy, EMA 0.999)
  - frac25_ema999 (ws=3000 then 25% off-policy mix, EMA 0.999)
  - frac50_ema999 (ws=3000 then 50% off-policy mix, EMA 0.999)
  - frac50_ema9999 (ws=3000 then 50% off-policy mix, EMA 0.9999)
"""
...
class AnalyticalValue(nn.Module):
    def __init__(self,means,sigma2,weights,a=1.0,c=None,D=2):
        ...
        self.register_buffer("weights",weights.float());self.a=a;self.D=D;self.register_buffer("c",c.float())
    def _log_Z(self,m,v):
...
def get_cond_mix(xt,ts,means,sigmas,weights,a):
    ...
    denom=2*a*(1-ts)+ts*sigmas_**2;le=-reduce((xt_-means_*ts_)**2,"n d m -> n m","sum")/(2*ts*denom)
    lsf=torch.log(2*a*(1-ts)/denom)*d/2;lrw=olw+le+lsf;lw=lrw-torch.logsumexp(lrw,dim=1,keepdim=True)
    lw=torch.where((ts==0),olw,lw);nm=(2*a*(1-ts_)*means_+xt_*sigmas_[None,...]**2)/denom[:,None,:]
...
def gmm_sample(n):
    k=np.random.choice(len(weights_np),size=n,p=weights_np);return means_np[k]+sigmas_np[k,np.newaxis]*np.random.randn(n,D)
...
LOADER_BATCH_SIZE=256;WS_STEPS=3000;ON_STEPS=3000;TOTAL_STEPS=WS_STEPS+ON_STEPS
LOG_DIR="lightning_logs/mixed_unified";CKPT_DIR="checkpoints/mixed_unified"
...
class TrajectoryEvalCallback(Callback):
    ...
    def on_validation_batch_end(self,trainer,pl_module,outputs,batch,batch_idx):
        ...
        for beta,label in [(0,"base"),(1,"guided")]:
            ...
            with torch.no_grad():vp=pl_module.value_module(all_x,all_t);va=self.anal_fn(all_x,all_t)
            err=vp-va
...
class FracScheduleCallback(Callback):
    """Switch off_policy_frac at a given step."""
    ...
    def on_train_batch_start(self, trainer, pl_module, batch, batch_idx):
        if not self.switched and trainer.global_step >= self.switch_step:
            ...
            self.switched = True
            print(f"  [step {trainer.global_step}] Switched off_policy_frac → {self.new_frac}")


def run_unified(run_name, ema_decay, on_frac):
    """Run with off_policy_frac=1.0 for WS_STEPS, then on_frac for ON_STEPS.

    Single model, single optimizer, single trainer — optimizer state preserved.
    """
    print(f"\n{'='*70}")
    print(f"  {run_name}: {TOTAL_STEPS} steps (ws={WS_STEPS} then frac={on_frac})")
    ...
    model = OnPolicyValue(
        ...
        reward_function=reward_fn, dim=D, a=a, lr=3e-3,
        loss_type="quad", analytical_value_fn=anal_fn,
        ema_decay=ema_decay,
    )

    # Dataset starts with off_policy_frac=1.0 (pure off-policy warm-start)
...
def run_offpolicy_baseline():
    rn = "pure_offpolicy"
    print(f"\n{'='*70}\n  {rn}: {TOTAL_STEPS} off-policy steps\n{'='*70}")
    ...
    model = OffPolicyValue(base_score_module=base_drift, reward_function=reward_fn,
        value_module=vm, dim=D, a=a, lr=3e-3, loss_type="quad", analytical_value_fn=anal_fn)
    traj_cb = TrajectoryEvalCallback(anal_fn)
    logger = CSVLogger(LOG_DIR, name=rn, version=0)
    trainer = L.Trainer(max_steps=TOTAL_STEPS, val_check_interval=max(1,TOTAL_STEPS//60),
...
def run_osb_blend():
    ...
    model = OnPolicyValue(base_score_module=base_drift, value_module=vm,
        reward_function=reward_fn, dim=D, a=a, lr=3e-3, loss_type="quad", analytical_value_fn=anal_fn)
    # Phase 1: pure off-policy with blend
    ...
    ds.raw_value_fn = ds.value_fn  # blend
    loader = DataLoader(ds, batch_size=LOADER_BATCH_SIZE)
    traj_cb = TrajectoryEvalCallback(anal_fn)
    ckpt_cb = ModelCheckpoint(dirpath=f"{CKPT_DIR}/{rn}", save_last=True, save_top_k=1,
        monitor="val_reward_mean", mode="max", filename="best")
...
# Execute
# ---------------------------------------------------------------------------
run_offpolicy_baseline()
run_osb_blend()
run_unified("frac0_ema999", ema_decay=0.999, on_frac=0.0)
run_unified("frac25_ema999", ema_decay=0.999, on_frac=0.25)
run_unified("frac50_ema999", ema_decay=0.999, on_frac=0.5)
run_unified("frac50_ema9999", ema_decay=0.9999, on_frac=0.5)
...
RUNS = ["pure_offpolicy", "osb_blend", "frac0_ema999", "frac25_ema999", "frac50_ema999", "frac50_ema9999"]

print(f"\n  {'Run':<20} {'Best':>8} {'Final':>8} {'Gap':>7} {'B-MAE':>7} {'G-MAE':>7}")
print(f"  {'-'*61}")
run_data = {}
for rn in RUNS:
    ...
    best=val["val_reward_mean"].max();final=val["val_reward_mean"].iloc[-1];gap=E_OPT-best
    ...
    print(f"  {rn:<20} {best:>8.3f} {final:>8.3f} {gap:>7.3f} {bf:>7.3f} {gf:>7.3f}")
    run_data[rn] = df

fig, ax = plt.subplots(figsize=(14,7))
ax.set_title("Mixed Training (Unified): Terminal Reward vs Steps", fontsize=13, fontweight="bold")
cmap=plt.cm.tab10;colors={rn:cmap(i/len(RUNS)) for i,rn in enumerate(RUNS)}
colors["pure_offpolicy"]="black";colors["osb_blend"]="red"
for rn in RUNS:
    if rn not in run_data: continue
    df=run_data[rn];val=df.dropna(subset=["val_reward_mean"])
    ls="--" if rn=="pure_offpolicy" else(":" if "osb" in rn else "-")
    lw=2.5 if rn in("pure_offpolicy","osb_blend") else 1.5
...
ax.axvline(WS_STEPS,color="gray",linestyle=":",alpha=0.3,label="ws→on transition")
ax.set_xlabel("Steps");ax.set_ylabel("Avg Terminal Reward");ax.legend(fontsize=7,loc="lower right");ax.grid(True,alpha=0.3)
plt.tight_layout();plt.savefig("experiments/misc/2026-05-06_mixed_training/mixed_unified_reward.png",dpi=150,bbox_inches="tight")
print(f"\nSaved: experiments/misc/2026-05-06_mixed_training/mixed_unified_reward.png");plt.close()
print(f"\nDone.")
