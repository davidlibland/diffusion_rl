"""
EMA experiment: does using EMA for the value function in OnPolicySMCDataset
stabilize training for ancestral_td_lambda and ancestral_mc_td_lambda?

All runs: 3000 off-policy warm-start → 3000 on-policy (6000 total).

Configurations:
  - offpolicy (baseline)
  - osb_blend_ws3000 (previous best: OSB + blended V + smc=reward)
  - ATD(λ=0) + smc=reward + ema value
  - ATD(λ=0) + smc=ema + ema value
  - ATD(λ_s=0.5) + smc=reward + ema value
  - ATD(λ_s=0.5) + smc=ema + ema value
  - AMCTD(λ=0) + smc=reward + ema value
  - AMCTD(λ=0) + smc=ema + ema value
  - AMCTD(λ_s=0.5) + smc=reward + ema value
  - AMCTD(λ_s=0.5) + smc=ema + ema value
"""
...
LOG_DIR = "lightning_logs/ema_exp"
CKPT_DIR = "checkpoints/ema_exp"
...
# Lambda values
LAM_0 = 0.0        # λ=0 (pure one-step)
LAM_S05 = 7.89e-31  # per-step λ = 0.5


# ---------------------------------------------------------------------------
# Trajectory eval callback (same as warmstart_final_exp)
# ---------------------------------------------------------------------------
...
def train_onpolicy(vm, smc_fn, sampling_method, lambda_eff, max_steps,
                   run_name, version=1, use_ema_value=False, use_blend=False):
    """Generic on-policy training.

    use_ema_value: if True, pass model.ema as value to the dataset
    use_blend: if True, monkey-patch raw_value_fn → value_fn (blended)
    """
    # We create the model first so we can reference model.ema
    model = OnPolicyValue(
    ...
    # Determine value function for dataset
    if use_ema_value:
        value_for_ds = model.ema
    else:
        value_for_ds = vm
...
RUNS = [
    ...
    dict(name="osb_blend", method="osb_blend",
         sampling_method="one_step_bootstrap", lam=0.0,
         smc="reward", use_ema=False, use_blend=True),

    # ATD with EMA value
    dict(name="atd0_ema_smc_rwd", method="onpolicy",
         sampling_method="ancestral_td_lambda", lam=LAM_0,
         smc="reward", use_ema=True, use_blend=False),
    dict(name="atd0_ema_smc_ema", method="onpolicy",
         sampling_method="ancestral_td_lambda", lam=LAM_0,
         smc="ema", use_ema=True, use_blend=False),
    dict(name="atds05_ema_smc_rwd", method="onpolicy",
         sampling_method="ancestral_td_lambda", lam=LAM_S05,
         smc="reward", use_ema=True, use_blend=False),
    dict(name="atds05_ema_smc_ema", method="onpolicy",
         sampling_method="ancestral_td_lambda", lam=LAM_S05,
         smc="ema", use_ema=True, use_blend=False),

    # AMCTD with EMA value
    dict(name="amctd0_ema_smc_rwd", method="onpolicy",
         sampling_method="ancestral_mc_td_lambda", lam=LAM_0,
         smc="reward", use_ema=True, use_blend=False),
    dict(name="amctd0_ema_smc_ema", method="onpolicy",
         sampling_method="ancestral_mc_td_lambda", lam=LAM_0,
         smc="ema", use_ema=True, use_blend=False),
    dict(name="amctds05_ema_smc_rwd", method="onpolicy",
         sampling_method="ancestral_mc_td_lambda", lam=LAM_S05,
         smc="reward", use_ema=True, use_blend=False),
    dict(name="amctds05_ema_smc_ema", method="onpolicy",
         sampling_method="ancestral_mc_td_lambda", lam=LAM_S05,
         smc="ema", use_ema=True, use_blend=False),
]
...
for cfg in RUNS:
    ...
    if method == "offpolicy":
        ...
        print(f"  {run_name}: {WS_STEPS} off → {ON_STEPS} on")
        print(f"    method={cfg['sampling_method']} λ={cfg['lam']}")
        print(f"    smc={cfg['smc']} ema_value={cfg['use_ema']} blend={cfg.get('use_blend', False)}")
    print(f"{'='*70}")
    ...
    # Determine smc_fn — need reference to model for ema
    # We'll create the model inside train_onpolicy and pass smc type
    if cfg["smc"] == "reward":
        smc_fn_for_ds = smc_reward
    elif cfg["smc"] == "ema":
        # Will be set inside train_onpolicy using model.ema
        smc_fn_for_ds = "ema"  # sentinel

    # Phase 2: on-policy
    print(f"  Phase 2: On-policy for {ON_STEPS} steps...")
    t0 = time.perf_counter()

    # For ema SMC, we need to create model first, then set smc
    if smc_fn_for_ds == "ema":
        # Create model to get ema reference
        model = OnPolicyValue(
            base_score_module=base_drift, value_module=vm,
            reward_function=reward, dim=D, a=a, lr=LR,
            loss_type="quad", analytical_value_fn=anal_fn,
        ...
        ds = OnPolicySMCDataset(
            dim=D, drift=base_drift,
            value=model.ema if cfg["use_ema"] else vm,
            smc_value=model.ema,
            reward=reward, device=DEVICE, a=a,
            batch_size=32, n_steps=100, mc_samples_per_step=10,
        ...
        logger = CSVLogger(LOG_DIR, name=run_name, version=1)
        trainer = L.Trainer(
        ...
        train_onpolicy(vm, smc_fn_for_ds, cfg["sampling_method"], cfg["lam"],
                       ON_STEPS, run_name, version=1,
                       use_ema_value=cfg["use_ema"],
                       use_blend=cfg.get("use_blend", False))
...
def load_combined(run_name, method):
    ...
    if method != "offpolicy":
        ...
        if os.path.exists(csv1):
            ...
            df1["step"] = df1["step"] + WS_STEPS
            dfs.append(df1)
    return pd.concat(dfs, ignore_index=True) if dfs else None


print(f"\n  {'Run':<28} {'Best Rwd':>9} {'Final Rwd':>10} {'Gap':>7} {'Base MAE':>9} {'Guided MAE':>11}")
print(f"  {'-'*78}")
...
for cfg in RUNS:
    ...
    base_sub = df.dropna(subset=["traj_avg_mae_base"])
    guided_sub = df.dropna(subset=["traj_avg_mae_guided"])
    base_mae = base_sub["traj_avg_mae_base"].iloc[-1] if len(base_sub) > 0 else float("nan")
    guided_mae = guided_sub["traj_avg_mae_guided"].iloc[-1] if len(guided_sub) > 0 else float("nan")

    print(f"  {rn:<28} {best:>9.4f} {final:>10.4f} {gap:>7.4f} {base_mae:>9.4f} {guided_mae:>11.4f}")
    run_data[rn] = df
...
ax.set_title("EMA Experiment: Terminal Reward vs Training Steps", fontsize=13, fontweight="bold")
...
for cfg in RUNS:
    ...
    lw = 2.5 if rn in ("offpolicy", "osb_blend") else 1.2
...
ax.axvline(WS_STEPS, color="gray", linestyle=":", alpha=0.3, linewidth=1)
ax.set_xlabel("Training Steps"); ax.set_ylabel("Avg Terminal Reward")
ax.legend(fontsize=6, loc="lower right", ncol=2); ax.grid(True, alpha=0.3)
plt.tight_layout()
plt.savefig("experiments/misc/2026-05-06_ema_experiment/ema_experiment_reward.png", dpi=150, bbox_inches="tight")
print(f"\nSaved: experiments/misc/2026-05-06_ema_experiment/ema_experiment_reward.png")
plt.close()
...
for ax_idx, (metric, title) in enumerate([
    ("traj_avg_mae_base", "Base drift (β=0)"),
    ("traj_avg_mae_guided", "Guided (β=1)"),
]):
    ax = axes[ax_idx]
    ax.set_title(title)
    for cfg in RUNS:
        ...
        ls = "--" if rn == "offpolicy" else (":" if "osb" in rn else "-")
        lw = 2.5 if rn in ("offpolicy", "osb_blend") else 1.2
        ax.plot(sub["step"].values, sub[metric].values, color=colors[rn],
                linestyle=ls, linewidth=lw, label=rn)
    ax.set_xlabel("Steps"); ax.set_ylabel("Avg MAE")
    ax.legend(fontsize=5, ncol=2); ax.grid(True, alpha=0.3); ax.set_yscale("log")
plt.tight_layout()
plt.savefig("experiments/misc/2026-05-06_ema_experiment/ema_experiment_mae.png", dpi=150, bbox_inches="tight")
print("Saved: experiments/misc/2026-05-06_ema_experiment/ema_experiment_mae.png")
plt.close()
