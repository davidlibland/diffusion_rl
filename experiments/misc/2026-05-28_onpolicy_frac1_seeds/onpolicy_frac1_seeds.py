#!/usr/bin/env python3
"""5 seeds of OnPolicySMCDataset with off_policy_frac=1.0 + no EMA in rollout.

Verifies that the on-policy *codebase* produces statistically identical
results to the off-policy codebase when the data-generation knob is
turned all the way off:

  off_policy_frac = 1.0  → every emitted (x, t, target) is replaced by
  a fresh draw from the off-policy interpolation:
        x1 ~ p,  t ~ U(0,1),  eps ~ N(0,I),
        x  = t·x1 + sqrt(2a t (1-t)) · eps,
        target = h(x1).

This is exactly what `InterpolatingNumpyDataset` produces, so combined
with `OnPolicyValueLive` (use_ema=False in drift), the resulting
distribution of best reward and final guided MAE should be statistically
indistinguishable from the off-policy 10-seed distribution.
"""
...
# ── Constants ──────────────────────────────────────────────────────────────
LOG_DIR = "lightning_logs/onpolicy_frac1_seeds"
CKPT_DIR = "checkpoints/onpolicy_frac1_seeds"
...
N_STEPS = 100
MC_SAMPLES = 10
DS_BATCH = 4              # internal trajectory batch (matches prior runs)
N_SEEDS = 5
...
def run_one(seed):
    ...
    ds = OnPolicySMCDataset(
        dim=D,
        drift=base_drift,
        value=model.ema,           # used in SSMC forward (will be discarded by frac=1)
        smc_value=log_tau_zero,
        reward=reward_fn,
        device=DEVICE,
        a=a,
        batch_size=DS_BATCH,
        ...
        off_policy_frac=1.0,         # ← every sample replaced with off-policy data
...
print(f"BS={BS}  LR={LR}  TOTAL_STEPS={TOTAL_STEPS}")
print(f"OnPolicySMCDataset(off_policy_frac=1.0)  +  OnPolicyValueLive (no EMA)")
...
# ── Aggregate + plot vs off-policy 10-seed distribution ──────────────────
...
print(f"\n{'=' * 72}")
print(f"  OnPolicy + off_policy_frac=1.0 + no-EMA  ({len(bests)} seeds)")
print(f"{'=' * 72}")
...
print(f"\n  Welch t-test  (off-policy vs OnPolicy frac=1)")
print(f"    best reward:      t={t_best:+.2f}  p={p_best:.3f}")
print(f"    final guided MAE: t={t_mae:+.2f}  p={p_mae:.3f}")
print("\n  Expected: p > 0.1 on both → distributions are statistically equivalent.")


# Plot
fig, axes = plt.subplots(2, 2, figsize=(14, 9))
fig.suptitle(
    f"Off-policy (n={len(off_val_curves)})  vs  OnPolicySMCDataset(frac=1) + no-EMA "
    f"(n={len(val_curves)})  —  1000 steps",
...
for val in val_curves:
    ax.plot(val["step"], val["val_reward_mean"], color=NEW_C, alpha=0.7, lw=1.4)
...
ax.plot([], [], color=NEW_C, lw=2, label=f"OnPolicy frac=1 (n={len(val_curves)})")
...
ax.hist(bests, bins=5, color=NEW_C, alpha=0.65, edgecolor="white",
        label=f"OnPolicy frac=1 n={len(bests)}")
...
for mae in mae_curves:
    if len(mae) > 0:
        ax.plot(mae["step"], mae["traj_avg_mae_guided"], color=NEW_C, alpha=0.7, lw=1.4)
...
ax.hist(maes, bins=5, color=NEW_C, alpha=0.65, edgecolor="white",
        label=f"OnPolicy frac=1 n={len(maes)}")
...
plt.tight_layout()
out = "experiments/misc/2026-05-28_onpolicy_frac1_seeds/onpolicy_frac1_seeds.png"
plt.savefig(out, dpi=140, bbox_inches="tight")
print(f"\nSaved: {out}")

with open("experiments/misc/2026-05-28_onpolicy_frac1_seeds/onpolicy_frac1_seeds_results.json", "w") as f:
    json.dump(
        {
            "config": {
                "TOTAL_STEPS": TOTAL_STEPS, "BS": BS, "LR": LR,
                "MC_SAMPLES": MC_SAMPLES, "DS_BATCH": DS_BATCH,
                "off_policy_frac": 1.0, "use_ema": False,
                "n_seeds": len(bests), "E_OPT": E_OPT,
            },
