"""Run a single mixed-frac config. Usage: python run_single_frac.py <frac>"""
import sys
frac = float(sys.argv[1])
rn = f"frac{int(frac*100)}"

# Import everything fresh (standalone process)
import gc, os, shutil, time, json
...
_avm=AV(_means,_sigma2,_weights,a=a,c=c,D=D)
def anal_fn(x,t):return _avm(x.cpu(),t.cpu()).to(x.device)

def gcm(xt,ts,means,sigmas,weights,a):
...
def gmm_drift(xt,ts,a):
    ts=ts.reshape(-1,1)
    cond=gcm(xt,ts,_means.float().to(xt),_sigmas.float().to(xt),_weights_col.float().to(xt),a)
...
LR=1e-3;EMA_DECAY=0.999;BS=256;WS=3000;TOTAL=6000
LOG_DIR="lightning_logs/mixed_final";CKPT_DIR="checkpoints/mixed_final"
...
class TrajCB(Callback):
    ...
    def on_validation_batch_end(self,trainer,pl,outputs,batch,bi):
        ...
        for beta,label in [(0,"base"),(1,"guided")]:
            ...
            err=vp-va;pl.log(f"traj_avg_mae_{label}",err.abs().mean(),prog_bar=False)

class FracCB(Callback):
    def __init__(self,ds,step,frac):super().__init__();self.ds=ds;self.step=step;self.frac=frac;self.done=False
    def on_train_batch_start(self,trainer,pl,batch,bi):
        if not self.done and trainer.global_step>=self.step:
            self.ds.off_policy_frac=self.frac;self.done=True
            print(f"  [step {trainer.global_step}] frac->{self.frac}")

print(f"Running {rn} (frac={frac}, LR={LR}, EMA={EMA_DECAY})")
for v in range(3):
    p=f"{LOG_DIR}/{rn}/version_{v}"
    if os.path.exists(p):shutil.rmtree(p)

vm=ValueNetwork(D,bias=bias_val)
model=OnPolicyValue(base_score_module=base_drift,value_module=vm,reward_function=reward_fn,
    dim=D,a=a,lr=LR,loss_type="quad",analytical_value_fn=anal_fn,ema_decay=EMA_DECAY)
ds=OnPolicySMCDataset(dim=D,drift=base_drift,value=model.ema,smc_value=smc_reward,
    reward=reward_fn,device=DEVICE,a=a,batch_size=4,n_steps=100,mc_samples_per_step=10,
    sampling_method="ancestral_td_lambda",lambda_eff=0.0,
    off_policy_frac=1.0,generating_function=gmm_sample,off_policy_batch_size=1024)
loader=DataLoader(ds,batch_size=BS)
tcb=TrajCB(anal_fn)
ccb=ModelCheckpoint(dirpath=f"{CKPT_DIR}/{rn}",save_last=True,save_top_k=1,
    monitor="val_reward_mean",mode="max",filename="best")
fcb=FracCB(ds,WS,frac)
logger=CSVLogger(LOG_DIR,name=rn,version=0)
trainer=L.Trainer(max_steps=TOTAL,val_check_interval=max(1,TOTAL//60),
    callbacks=[ccb,tcb,fcb],logger=logger,enable_checkpointing=True,enable_progress_bar=True)
t0=time.perf_counter()
trainer.fit(model,loader,val_dataloaders=val_loader)
print(f"Elapsed: {(time.perf_counter()-t0)/60:.1f} min")

df=pd.read_csv(f"{LOG_DIR}/{rn}/version_0/metrics.csv")
...
print(f"Result: best={best:.3f} final={final:.3f} base_mae={bf:.3f} guided_mae={gf:.3f}")
