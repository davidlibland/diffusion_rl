...
demonstrably better than controlling one. It does not dominate the family: unclipped Itakura--Saito
estimates the value better above $d=2$. The durable contribution is therefore
the reduction itself --- that loss design here is the choice of a single
positive weight, and that squared error is a poor choice of it --- and not a
claim that our weight is the best available one.

The clipped comparison, which would settle whether tail control matters once
the standard remedy is applied, is unresolved: our first attempt used a
threshold that bound on every step and so compared normalised-gradient descent
rather than the losses (\S\ref{par:clipped}). The per-arm rerun is the single
experiment we would most want completed before drawing a practical
recommendation.
