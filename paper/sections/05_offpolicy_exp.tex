...
\eqref{eq:jensen-gap} predicts. A loss can be well conditioned and wrong. For
an application that reads only $\nabla \Vf$ --- which includes reward-guided
generation and fine-tuning, the settings of \S\ref{sec:01_intro} --- that trade
is available and cheap, and our results say to take it. The unbiased target
earns its keep when $\Vf$ itself is the object, and since $\Vf(0,0)=\log Z$
that includes every free-energy and log-partition use.
